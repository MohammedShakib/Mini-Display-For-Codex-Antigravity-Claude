...
- If you generated a token after the hidden loop was already running, restart the loop so the process inherits the new environment variable:
  ...
  wscript.exe ".\start_codex_limit_clock_hidden.vbs"
  ```
- The GitHub card's `Today` count uses push-visible commit activity across the user's recent GitHub repositories and known local remote-tracking repos, not the profile contribution graph, so forgotten-push days can still show `No activity today`.
